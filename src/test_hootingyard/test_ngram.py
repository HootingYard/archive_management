...
def test_g_ngrams_with_words():
    ...
    result = get_ngrams(ngram_length=3, max_ngrams=4, wf_scoring_function=scoring_function, filter_function=filter_function, word_iterator=iter(source_text))

    assert result == ['we have forgotten', 'time before time', 'the time before', 'before time is']
