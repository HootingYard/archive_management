...
def heading(element):  # type: (HtmlElement) -> Text
    r""" Returns a chapter heading. The "squeezeN" and "padN" classes allow a few lines
...
def inline(surrounding_element, footnotes=True):  # type: (HtmlElement, bool) -> Text
    ...
    for element in surrounding_element.iterchildren("*"):
        ...
        if tag == 'br':
            ...
            s += '\\emph{' + inline(element, footnotes) + '}'
        elif tag == 'tt':
            s += '\\texttt{' + inline(element, footnotes) + '}'
        elif tag == 'small':
            s += '{\\small ' + inline(element, footnotes) + '}'
        elif tag == 'sup':
            s += '\\textsuperscript{' + inline(element, footnotes) + '}'
        elif tag == 'del':
            s += '\\sout{' + inline(element, footnotes) + '}'  # from LaTeX package 'ulem'
        elif tag == 'a':
            s += link(element, footnotes)
        elif tag == 'strong':
            s += '\\textbf{' + inline(element, footnotes) + '}'
        elif tag == 'small':
            s += '{\\small ' + inline(element, footnotes) + '}'
        else:
