...
import re, time, os, uuid, codecs, json
import asciify
...
def isempty (element):  # type: (HtmlElement) -> bool
    ...
    if NonEmpty.search(element.text or ''): return False
    for e in element:
        if e.tag == 'img': return False
        if e.tail is not None and NonEmpty.search(e.tail): return False
        if not isempty(e): return False
    return True
