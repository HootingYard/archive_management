...
def get_config(config_file_path="~/.config/hootingyard/config.yaml"):
    expanded_config_file_path:str = os.path.expanduser("~/.config/hootingyard/config.yaml")
    with open(expanded_config_file_path) as config_file:
        try:
            return Config(**yaml.load(config_file))
        except TypeError:
            log.exception(f"Missing attribute in config file: {expanded_config_file_path}")
            raise
